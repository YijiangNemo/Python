...
try:
    ...
    print('File {} could not be found.'.format(image_filename))
except OSError:
    print('File {} could not be processed as an image file.'.format(image_file))
...
with open(tex_filename, 'w') as tex_file:
    ...
    for i in range(height):
        ...
        for j in range(width):
            ...
            # be surrounded by a given character. As we want to be able to output any ascii character,
            # we do not choose an ascii character for that surrounding character; we (arbitrarily)
            # choose the character obtained on a Mac by pressing Option 0.
            # Note the use of \\, {{ and }} to output literal \, { and }, respectively.
            print('\\node at ({}, {}) {{\\verbº{}º}};'.format(j, i, alphabet[int((
                     mean(tuple(image.crop((x, y, x + nb_of_pixels, y + nb_of_pixels)).getdata()))
                                  - min_luminance) / luminance_range * len(alphabet))]), file = tex_file)
...
for file in (ascii_image_name + ext for ext in ('.aux', '.log')):
    os.remove(file)
print('\nProduced {0:}, replacing blocks of {1:}x{1:} pixels by one character.'
                                                        .format(ascii_image_name + '.pdf', nb_of_pixels))
